...
Deploy NSC and NSE:
```bash
kubectl apply -k ../../../examples/heal/remote-nsmgr-death/remote-nse
```
...
Kill remote NSMgr:
```bash
kubectl apply -k ../../../examples/heal/remote-nsmgr-death/nsmgr-death
```

Start local NSE instead of the remote one:
```bash
kubectl apply -k ../../../examples/heal/remote-nsmgr-death/local-nse
```
