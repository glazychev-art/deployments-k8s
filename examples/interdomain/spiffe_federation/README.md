...
```bash
kubectl --kubeconfig=$KUBECONFIG1 apply -f ../../../examples/interdomain/spiffe_federation/cluster1-spiffeid-template.yaml
```

Apply the ClusterSPIFFEID CR for the second cluster:
```bash
kubectl --kubeconfig=$KUBECONFIG2 apply -f ../../../examples/interdomain/spiffe_federation/cluster2-spiffeid-template.yaml
```
